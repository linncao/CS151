...
def frame_test(x, y, scale):
    """Tests the frame
    """
    screen = gr.GraphWin('Painting Gallery, Artwork 1', width=1000, height=700)
...
def text_init(x, y, scale):
    ...
    """
    shapes = []
    message1 = gr.Text(gr.Point(x, y), 'Artwork: Flappy Bird\nArtist: Linn Cao\nCreated on Oct 20, 2020')
...
def wall_init(x, y, scale):
    ...
    wall2 = gr.Rectangle(gr.Point(x + 100*scale, y + 500*scale), gr.Point(x + 1000*scale, y + 700*scale))
    wall2.setFill(gr.color_rgb(255, 250, 250))
    wall2.setOutline(gr.color_rgb(255, 250, 250))
    wall3 = gr.Rectangle(gr.Point(x, y), gr.Point(x + 1000*scale, y + 220*scale))
    wall3.setFill(gr.color_rgb(255, 250, 250))
    wall3.setOutline(gr.color_rgb(255, 250, 250))
    wall4 = gr.Rectangle(gr.Point(x + 500*scale, y), gr.Point(x + 1000*scale, y + 700*scale))
...
def main():
    ...
    bird = cs.bird_init(140, 380, 1.8)
    pipes = cs.pipes_init(216, 216, 0.4)
    background = cs.background_init(96, 460, 0.8)
    ...
    for i in range(70):
        ...
        cs.bird_animate(bird, i, screen, 0.4)
        time.sleep(0.1)
